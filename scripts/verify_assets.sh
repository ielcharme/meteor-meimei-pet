...
verify_app() {
  ...
  [[ "$version" == "1.4" ]] || { echo "Unexpected app version: $version" >&2; return 1; }
  ...
  [[ "$behavior_config" == *"corner_hide_seconds=300"* && "$behavior_config" == *"hover_reveal=jump-to-corner"* && "$behavior_config" == *"left_source=running-right-mirrored"* ]] || { echo "Unexpected behavior config: $behavior_config" >&2; return 1; }
