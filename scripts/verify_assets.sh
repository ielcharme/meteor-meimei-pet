...
verify_app() {
  ...
  [[ "$version" == "1.6" ]] || { echo "Unexpected app version: $version" >&2; return 1; }
  ...
  [[ "$behavior_config" == *"corner_hide_seconds=300"* && "$behavior_config" == *"hover_reveal=jump-to-corner"* && "$behavior_config" == *"focus_protection=typing-fullscreen-media"* && "$behavior_config" == *"cinema_mode=manual"* && "$behavior_config" == *"wellness_interval_seconds=3600"* && "$behavior_config" == *"wellness_display_seconds=10"* && "$behavior_config" == *"work_active_window_seconds=300"* && "$behavior_config" == *"left_source=running-right-mirrored"* ]] || { echo "Unexpected behavior config: $behavior_config" >&2; return 1; }
